...
Under 7 has a separate parent/guardian form: child-view mode offers four response boxes and a separate optional guardian-observation box, with the child’s willingness confirmed first; observation-only mode has one observation field and no invented child assent. Its `young_child_supported` record is distinct from schema 6.0 support-area records and must not be pooled as equivalent child self-report. See [CHILD_PARTICIPATION_REVIEW.md](CHILD_PARTICIPATION_REVIEW.md). The separate contact form has its own minimal all-age routes described in [CONTACT_FORM.md](CONTACT_FORM.md).
