...
The finder starts with eight everyday tasks, then asks only the relevant context on one form. Results show one first contact and at most two alternatives, with age/relationship/place suitability, exact free scope, opening hours and official contact links. The previous category tree and 39-need research inventory are not loaded by the page. Urgent help remains in the footer; safety situations have contextual contacts.

Maintain `support-catalog.mjs` from official provider evidence, and update `support-model.mjs` for task-specific questions and matching. Service records include checked dates and source URLs. Do not infer entitlement from a broad family label, send a child to an adult centre, promise a childcare place or bed, or call all onward care free. Recheck time-sensitive outage/address notices before referrals. Run `node --test tests/*.test.mjs` before publishing. `tests/support.test.mjs` verifies real help-seeking scenarios and all route combinations. See `SUPPORT_DESIGN.md` for the new design and verification.
