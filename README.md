...
The finder starts with four circumstance routes. Moving, separation and leaving routes open contact-ready results after one further choice. Specific concerns open five short groups, each with at most five distinct needs, while safety remains a direct choice; contacts appear after at most three choices from the start. A smaller human-help link works for people unsure where to begin. `support-journeys.mjs` holds the reader-facing route-to-service selections. The earlier 39 headings remain internal source tags in `support-data.mjs`, not a public questionnaire, validated taxonomy or measure of NT prevalence. Results lead with a suitable first contact where one can be identified, show audience and area visibly, then provide concise alternatives and local phone lists. Language, inclusion, privacy, feedback and urgent contacts sit in the footer. There is no topic search or stored finder choice. Official source pages were checked on 25 September 2026; recheck time-sensitive availability before making referrals.

To maintain the resource, update the official provider URL and access wording in `support-data.mjs`, then place its ID in the relevant `support-journeys.mjs` choice. Keep the first set of contacts short and use `moreServiceIds` for conditional alternatives. Run `node --test tests/*.test.mjs` before publishing; `tests/support.test.mjs` checks all route and service references.
