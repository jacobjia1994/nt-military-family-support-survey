...
The finder starts with recognisable life situations rather than an exclusive six-area taxonomy or 39 questions. Clear situations open a short service list in one choice; broader situations ask one more specific question. `support-journeys.mjs` defines the reader-facing routes and intentional cross-links. The 39 headings from the supplied *NT Defence Support - 39 challenges and relevant support* reference remain internal need tags in `support-data.mjs`, with source PDF page numbers and provider links. Feedback on service design, language/privacy help and urgent contacts sit in the footer. There is no topic search or stored finder choice. Service descriptions were checked against official provider pages on 25 September 2026; confirm time-sensitive details with the provider.
