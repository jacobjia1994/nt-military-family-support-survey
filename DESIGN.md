...
`support.html` extends the Lutheran Care visual system with the unchanged logo, self-hosted Karla and terracotta action colour. The home screen contains four equal circumstance routes and one smaller direct human-help link. It has no search, descriptive card grid or generic instruction sentence. The three life-situation routes show concrete next choices; the specific-concern route shows five short groups and a direct safety path, with at most five atomic needs per group. The longest path takes three choices before contacts appear.

Results lead with one visually prominent, source-supported first contact when a broadly suitable one exists. Its audience, purpose, area and decisive access limit are visible beside the call or official-site action. Up to three alternatives use simpler text rows; a named native disclosure holds further specialist options. For homelessness, regional phone contacts appear before the slower online intake, with the outage and access limits visible. Results with regional services show compact, area-labelled phone rows rather than an arbitrary Darwin-first stack of full cards. A direct human-help link remains available beneath each result. Footer links provide language, inclusion, privacy, participation feedback and urgent contacts. Immediate danger and safer-device advice appear within safety or urgent mental-health results. Legacy `#area`, `#need` and previous situation links remain readable. The respondent questionnaire and interview form retain their separate flows.
