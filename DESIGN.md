...
`support.html` extends the Lutheran Care visual system with the unchanged logo, self-hosted Karla and terracotta action colour. The support results show provider, purpose, location, a decisive limit where needed and a contact action; full audience and access details open on request.

On 25 September Jacob directed a further correction: urgent contacts belong in the footer, not above the main choice; the generic “Choose a topic” instruction and “Search all topics” control are removed. The six-area route remains only as the current interim navigation while its underlying logic is researched. The replacement should let a family recognise its situation without guessing an exclusive category, and reach a suitable service in a few clicks.
