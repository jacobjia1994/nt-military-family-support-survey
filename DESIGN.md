...
`support.html` extends the Lutheran Care visual system with its unchanged logo, self-hosted Karla and terracotta action colour. One column of situation statements is the primary navigation. The page has no search, descriptive card grid or generic instruction sentence. Footer links provide language/privacy help, participation feedback and urgent contacts without competing with the main task.

A direct journey shows a short, curated “Start here” service set, followed by optional specific situations. Broader journeys show only concrete next choices. One choice leads to a service result; the same need may appear in several journeys. The service result leads with provider, purpose, location, a decisive access limitation and call or website action. Full audience and access detail is a native disclosure. Contextual danger advice remains visible on the unsafe and urgent mental-health journeys. The respondent questionnaire and interview form retain their separate flows.
