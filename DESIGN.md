...
Replaced on 26 September 2026. Mode: Operate. Preserve the Lutheran Care logo and self-hosted Karla, while using a wider, unboxed reading surface for the support task. Restrained off-white background and terracotta contact actions; the support palette is defined locally in `support.css` and does not alter questionnaire tokens. Print uses #111 for legibility. A white or pale neutral contact panel distinguishes the primary action without presenting every service as a card.

The first viewport contains a short purpose sentence, eight concrete tasks arranged in two ruled columns on desktop and one column on mobile, and a human-help link. No hero, illustrations, dashboard, search or policy taxonomy. Tasks open one short context form; age and service questions appear only where they change matching. Native selects, full-width mobile submit button and visible keyboard focus carry the interaction.

Results display the primary service name, relevant place, help offered, audience, free scope and access requirements. Its contact action and hours sit beside it on desktop and directly below on mobile. Up to two alternatives use plain rows. The user's task/place/age context can be edited without losing current choices. A disclosure provides a concrete first-call sentence. Task navigation moves focus to the heading; the next Tab reaches the primary contact. The footer holds access/privacy guidance and urgent contacts. Mobile is checked at 390px and 320px; desktop uses the same content without decorative expansion.

See `SUPPORT_DESIGN.md` for source-driven route decisions and scenario verification. This replacement applies only to `support.html`; earlier questionnaire design remains current.
