...
`support.html` is a standalone resource for NT Defence members, veterans and families. It requires no survey participation or personal details. On 26 September 2026 Jacob requested a full rethink of the support page and explicitly authorised implementation, GitHub Pages publication and live verification. The previous four-entry/five-group structure is superseded.

The finder now starts with eight everyday tasks and a human-navigation option. A single short, task-specific form establishes the relevant location, recipient age and service connection before showing a first contact and up to two alternatives. The broad family definition in the national Strategy is not treated as a program entitlement. Unknown service history or extended-family relationships retain ordinary community support and a specific eligibility-check route. No personal choices are stored or transmitted, and only task names appear in the URL.

The 52-page Defence and Veteran Family Wellbeing Strategy and First Action Plan were read in full. Its nonlinear service/family lifecycles, direct family communication and mix of community/Defence support inform this design. The earlier 39 challenges remain research history; they do not define the public menu or constrain coverage. Policy intentions are not proof of a current service.

Primary service records live in `support-catalog.mjs`; task questions and matches in `support-model.mjs`; rendering in `support.js`. `support-data.mjs` and `support-journeys.mjs` are retained historical research, not loaded by the new page. Urgent contacts remain in the footer, with relevant safety advice only on safety/urgent-housing paths. The survey and interview forms are unchanged. See `SUPPORT_DESIGN.md` for evidence and validation.
