...
function reconcileAnswers(answers,changed,domains,previous) {
  if(changed==='needs_status'&&!hasNeedSelection(answers)){delete answers.needs;delete answers.needs_other;delete answers.areas;}
  if(changed==='needs') {
    const ids=selectedNeeds(answers,domains);
    if(!ids.includes('other_need'))delete answers.needs_other;
    answers.areas=Object.fromEntries(Object.entries(answers.areas||{}).filter(([id])=>ids.includes(id)));
  }
  ...
  if(changed==='serving_nt'&&previous!==answers.serving_nt) {
    for(const key of ['needs_status','needs','needs_other','areas','delivery','times','anything','earlier_experience'])delete answers[key];
  }
...
const period = () => isChild()?'the past three months':'the past 12 months';
...
function areaBarrierField(need) {
  ...
  return field(`areas:${need}:barriers`,notSought?(child?'Why have you not asked for help with this?':'What were your reasons for not looking for support?'):'What made it harder to get that support?','multi',[...list.filter(o=>!['none','unsure','prefer','prefer_not_to_say'].includes(o.id)),{id:'none',label:notSought?'None of these reasons':child?'Nothing made it hard':'Nothing made it harder'},UNSURE,PREFER],'Select all that apply.',{exclusive:['none','unsure','prefer'],need,optional_detail:true,conditional:'area_barriers'});
}
function areaPage(need) {
  ...
  return {title:domainLabel(need),intro:'',fields:[
    ...
    field(key('comment'),isChild()?'What happened when you needed help with this?':'What happened when you needed support with this?','text',[],(isChild()?'You can tell us what helped, or what could have helped. ':'You could describe what helped, or what would have made things easier. ')+privacyHint(),{need,optional_detail:true})
  ]};
}
function page(step) {
  ...
  switch(step.id) {
    ...
    case 'needs':return {title:child?'Where have you needed help?':'Your support needs',intro:child?'Help can come from people you know or people whose job is to help.':'Support can include help from family, friends, your community or a service.',fields:[
      ...
      field('needs',child?'What did you need help with?':'What did you need support with?','multi',domainList(),child?'Select all that apply. Include help you got and help you still need.':'Select all that apply. Include needs that were met and support you still need now.',{conditional:'needs_list'}),
...
function renderYoung(){
  if(!state.guardianPermission?.agreed||state.guardianPermission.age_path!=='young'){resetYoung();renderGuardianPermission();return;}
  state.screen='young';
  state.youngController ||= window.SURVEY_YOUNG_CHILDREN.create({main,guardianPermission:()=>state.guardianPermission,prompts:YOUNG_PROMPTS,onPermissionRequired:renderGuardianPermission,onBack:()=>{renderGuardianPermission();focusHeading();},onStop:renderParticipationDeclined,onInformation:()=>document.querySelector('#privacy-dialog').showModal(),onFinish:(record)=>{state.youngRecord=record;renderFinish();}});
...
function reviewHTML(){return activeSteps().filter(s=>s.id!=='review').map(s=>{
  const p=page(s);
  const rows=p.fields.filter(f=>conditionalVisible(f)&&(!f.optional_detail||hasAnswer(getValue(f.key)))).map(f=>{
...
function renderSurvey(){
  ...
  main.innerHTML=`<div class="survey-layout"><section class="survey-main"><div class="step-topline"><strong>${esc(displayPhases[phaseIndex])}</strong><span>Section ${phaseIndex+1} of ${displayPhases.length}</span></div><div class="section-track" aria-hidden="true">${displayPhases.map((_,i)=>`<span class="${i<=phaseIndex?'visited':''}"></span>`).join('')}</div><form class="question-card" id="survey-form" novalidate><h1 tabindex="-1">${esc(p.title)}</h1>${p.intro?`<p class="question-intro">${esc(p.intro)}</p>`:''}${s.need?`<p class="need-progress">Area ${selectedNeeds(state.answers,domainList()).indexOf(s.need)+1} of ${selectedNeeds(state.answers,domainList()).length}</p>`:''}${review?reviewHTML():s.need?areaQuestionsHTML(p,s.need):p.fields.filter(f=>f.key!=='needs_other').map(fieldHTML).join('')}<div class="error" id="form-error" role="alert"></div><div class="question-actions"><button class="back-button" type="button" id="back">Back</button><div class="action-right"><button class="button primary" type="submit">${review?'Finish':'Continue'}</button></div></div></form><button class="text-button" id="survey-help" type="button">Help or stop</button></section></div>`;
...
function questionLibrarySections(version,location) {
  ...
  try {
    ...
    add('needs','Ask whether support was needed first. No skips the area questions but retains service-information preferences. Yes or Not sure opens the area list; Something else is only an unlisted need. Blank remains distinct from No.');
