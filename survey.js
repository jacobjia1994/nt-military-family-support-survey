...
function renderWelcome(){
  ...
  main.innerHTML=`<div class="welcome"><section class="welcome-intro"><h1 tabindex="-1">Help shape support for Defence families</h1><p class="lead">Lutheran Care is developing its Defence family support program in the Northern Territory, with funding from the Australian Government Department of Defence.</p><p>We want to help families build local connections and find support that fits Defence life.</p><p>Tell us what’s working, what’s missing and what would make life here easier. Your answers will help shape the activities and support Lutheran Care offers.</p><p>You don’t need to be looking for help to take part. Each family member can share their own views.</p></section><div class="welcome-start"><button class="button primary" id="start-questionnaire">Start questionnaire <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M5 12h14m-6-6 6 6-6 6"/></svg></button><p>Taking part is voluntary.<br> You can skip most questions or stop at any time.</p></div></div>`;
...
function renderSurvey(){
  ...
  main.querySelector('#back').onclick=()=>{state.returnToReview=false;const current=activeSteps();const i=current.findIndex(x=>x.id===s.id);if(i<=0){renderAge();focusHeading();return;}state.step=current[i-1].id;renderSurvey();focusHeading();};
