...
function reconcileAnswers(answers, changed, domains, previous) {
  ...
  if(changed === 'priority' && !selectedPriorities(answers).includes('another')) delete answers.another_priority;
  if(changed === 'another_priority') {
    for(const key of ['impact','help','barriers','change','delivery','times']) delete answers[key];
  }
...
function cleanExport(answers, version, domains) {
  ...
  for(const key of ['caring','financial_dependence','care_dependence']) delete copy[key];
  ...
  return {schema_version:'2.0',questionnaire_revision:'2026-09-24-concise-multiple-needs',follow_up_scope:'selected_current_needs_collectively',collection_mode:'internal_review_no_transmission',questionnaire_version:version,storage:'downloaded_by_respondent; not submitted',answers:copy};
}
...
const SURVEY_INVITATION = {"title": "Defence family support survey", "greeting": "Hello,", "paragraphs": ["Lutheran Care would like your help to plan its Defence family support program in the Northern Territory.", "Tell us about the support you have needed, what you received and what would help now.", "Family members can take part wherever they live. Please answer about your own experience."], "funding": "The program is funded by the Australian Government Department of Defence through its Family Support Funding Program."};
const PARTICIPANT_NOTICE_VERSION = '2026-09-24-v8';
...
const PARTICIPANT_INFORMATION = [
  ['Your choice', 'Taking part is voluntary. Most questions are optional. Some ask about wellbeing, disability or safety.'],
  ...
  ['Use and storage', 'Lutheran Care will use your answers to plan its NT Defence family program. We will keep them in approved systems, limit access to authorised people working on the project, and retain and dispose of records under our records requirements.'],
  ...
  ['Withdrawal', 'You can stop before submitting. We do not keep a record linking answers to your name or contact details, so you cannot withdraw your response after submission.'],
...
function page(step) {
  ...
  switch(step.id) {
    case 'connection': return {title:isAdult()?'Your connection to military life':'A little about your family',intro:'',fields:[
      ...
      field('serving_nt',isAdult()?'Are you, or the military member in your family, currently serving in the NT?':'Is that person serving in the NT now?','single',opts([['yes','Yes'],['no','No'],['unsure','Not sure']]),'Select one.',{required:true}),
    ...
    case 'strengths': return {title:child?'What is going well?':'What already helps?',intro:locationFrame(a.region)!=='nt'?'Think about life while you or someone in your family is serving in the NT.':'',fields:[field('strengths',child?'What is going well in your life, or helps you feel supported?':locationFrame(a.region)!=='nt'?'What already helps you manage everyday life while you or someone in your family is serving in the NT?':'What already helps you manage everyday life in the NT?','text',[],privacyHint()) ]};
    ...
    case 'barriers': {
      ...
      return {title:notSought?(child?'About asking for help':'What influenced your decision?'):'What has made it hard to get help?',intro:'',fields:[field('barriers',notSought?'Which reasons apply to you?':neutral?`If you have tried to get help with ${needPhrase}, what made it difficult?`:`What made it harder to get help with ${needPhrase}?`,'multi',[...list.filter(o=>!['none','unsure','prefer','prefer_not_to_say'].includes(o.id)),{id:'none',label:notSought?'None of these reasons':child?'Nothing made it hard':'Nothing made it harder'},UNSURE,PREFER],'Select all that apply.',{exclusive:['none','unsure','prefer']})]};
    }
...
function conditionalVisible(f) { return f.conditional==='nt'?locationFrame(state.answers.region)==='nt':f.conditional==='other_need'?state.answers.needs?.includes('other_need'):f.conditional==='another'?selectedPriorities(state.answers).includes('another'):f.conditional==='live'?(state.answers.delivery||[]).some(v=>['one_to_one','group','phone','video'].includes(v)):true; }
...
function renderSurvey(){
  ...
  main.innerHTML=`<div class="survey-layout"><section class="survey-main"><div class="step-topline"><strong>${esc(phases[s.phase])}</strong><span>Section ${s.phase+1} of 5</span></div><div class="section-track" aria-hidden="true">${phases.map((_,i)=>`<span class="${i<=s.phase?'visited':''}"></span>`).join('')}</div><form class="question-card" id="survey-form" novalidate><h1 tabindex="-1">${esc(p.title)}</h1>${p.intro?`<p class="question-intro">${esc(p.intro)}</p>`:''}${!review&&['impact','help','barriers','change','delivery'].includes(s.id)?`<details class="selected-needs"><summary>Your selected needs (${selectedPriorities(state.answers).length})</summary><p>${esc(priorityLabel())}</p></details>`:''}${review?reviewHTML():p.fields.map(fieldHTML).join('')}<div class="error" id="form-error" role="alert"></div><div class="question-actions"><button class="back-button" type="button" id="back">Back</button><div class="action-right"><button class="button primary" type="submit">${review?'Finish':'Continue'}</button></div></div></form><button class="text-button" id="survey-help" type="button">Help or stop</button></section></div>`;
...
function questionLibrarySections(version, location) {
  ...
  try {
    ...
    add('place','Residence is optional and never an eligibility test; skipping or declining uses neutral wording.');
    add('strengths');
    ...
    add('priority','All current support areas are available, independently of past needs. Multiple selections lead to one set of collective follow-up questions. No current needs, unsure, declined or blank skips follow-up.');
    add('impact','Shown when one or more current needs are selected. The answer concerns those needs together.');
    add('help','Shown when one or more current needs are selected. The answer concerns those needs together.');
    const variants = [];
    ...
    add('change','Shown when one or more current needs are selected. The answer concerns those needs together.');
    if(version!=='child') add('delivery','Shown when one or more current needs are selected. The answer concerns those needs together. The timing question appears only for in-person, group, phone or video support.');
    add('anything');
    return sections;
